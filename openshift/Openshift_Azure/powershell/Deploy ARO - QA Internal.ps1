...
$zone="dev-circlek.westeurope.aroapp.io"

#Add-DnsServerPrimaryZone -Name $Zone -ZoneFile ($Zone + ".dns")

# Define RG and Cluster name
$ARO_RG="eur-modrse-ocp-rg"
$ARO_Name="aro-dev-weu-ocp-int"
$ARO_Location="westeurope"
$ARO_VNetName="prod-arocluster-weu-vnet"
$ARO_VNet_Subnet_CP="master-dev"
$ARO_VNet_Subnet_WRK="worker-dev"
$ARO_Service_Principal= "eur-modrse-cluster-prod-sp"
$ARO_Vault_Name= "eur-modrse-infra-kv"
$ARO_Vault_Secret = "eur-modrse-prod-sp-secret"
$ARO_Client_ID=""
$ARO_Client_Secret=""
...
# Create VNET
#az network vnet create -g $ARO_RG -n $ARO_VNetName --address-prefixes 10.87.117.0/24

# Create Subnets
az network vnet subnet create -g $ARO_RG --vnet-name $ARO_VNetName -n $ARO_VNet_Subnet_CP --address-prefixes 10.87.117.160/27
az network vnet subnet create -g $ARO_RG --vnet-name $ARO_VNetName -n $ARO_VNet_Subnet_WRK --address-prefixes 10.87.117.192/27
...
$ARO_Client_Secret = "3xI8Q~Zg1MAVVDh2OTn1vLrvWpjEUZLXMUKkDbI3"
